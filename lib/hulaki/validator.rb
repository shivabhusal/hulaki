class Hulaki::EmailValidator
  ...
  def validates_format
    ...
    @errors.store("sender_email_format", "email format not valid") if valid_format.match(@sender_email).nil?
    @errors.store("recipient_email_format", "email format not valid") if valid_format.match(@reciepient_email).nil?
    @errors
  end
  private
